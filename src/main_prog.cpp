...
void pre_periferal_config() {
  log_debug(
      stmepic::Logger::parse_to_json_format("state", "pre_perifial_config"));
...
void init_and_set_movement_controler_mode(uint8_t mode) {
  ...
  motor = new stmepic::motor::MotorClosedLoop(stp_motor, &encoder_arm,
                                              engine_encoder, nullptr);
  ...
  case CAN_KONARM_1_SET_CONTROL_MODE_CONTROL_MODE_POSITION_CONTROL_CHOICE:
    ...
    // control
    movement_controler.init(*motor,
                            stmepic::movement::MovementControlMode::VELOCITY,
                            bacis_controler);
    break;
  ...
  case CAN_KONARM_1_SET_CONTROL_MODE_CONTROL_MODE_TORQUE_CONTROL_CHOICE:
    ...
    movement_controler.init(*motor,
                            stmepic::movement::MovementControlMode::TORQUE,
                            pass_through_controler);
    break;
  default:
...
void post_id_config() {
  ...
  encoder_arm.set_dead_zone_correction_angle(
      config.encoder_arm_dead_zone_correction_angle);
  ...
  encoder_arm_filter_velocity.set_samples_to_skip(
      config.encoder_arm_velocity_sample_amount);
  encoder_arm_filter_velocity.set_init_value(0);
  encoder_arm.init(hi2c1, stmepic::encoders::translate_reg_to_angle_MT6701,
  ...
  encoder_vel_motor.set_dead_zone_correction_angle(
      config.encoder_motor_dead_zone_correction_angle);
  ...
  encoder_motor_moving_avarage.set_size(
      25); // 15 for smooth movement but delay with sampling to 50
  encoder_motor_moving_avarage.set_samples_to_skip(
      config.encoder_motor_velocity_sample_amount);
  encoder_vel_motor.init(hi2c1,
                         stmepic::encoders::translate_reg_to_angle_MT6701,
  ...
  movement_controler.set_limit_position(config.movement_limit_lower,
                                        config.movement_limit_upper);
  ...
  log_debug(stmepic::Logger::get_instance().parse_to_json_format(
      "state", "post_id_config"));
  ...
  can_controler.set_filter(config.can_filter_id_high,
                           config.can_filter_mask_high);
...
void error_checks() {
  error_data.temp_board_overheating =
      !std::isnan(temoperature_board) &&
              temoperature_board > ERRORS_MAX_TEMP_BOARD
          ? true
          : false;
  error_data.temp_driver_overheating =
      !std::isnan(temoperature_steper_driver) &&
              temoperature_steper_driver > ERRORS_MAX_TEMP_DRIVER
          ? true
          : false;
  error_data.temp_engine_overheating =
      !std::isnan(temoperature_steper_motor) &&
              temoperature_steper_motor > ERRORS_MAX_TEMP_ENGINE
          ? true
          : false;
  error_data.temp_board_sensor_disconnect = std::isnan(temoperature_board);
  error_data.temp_driver_sensor_disconnect =
      std::isnan(temoperature_steper_driver);
  error_data.temp_engine_sensor_disconnect =
      std::isnan(temoperature_steper_motor);

  error_data.encoder_arm_disconnect =
      !encoder_arm.device_is_connected().valueOrDie();
  error_data.encoder_motor_disconnect =
      !encoder_vel_motor.device_is_connected().valueOrDie();

  error_data.baord_overvoltage =
      voltage_vcc > ERRORS_MAX_VCC_VOLTAGE ? true : false;
  error_data.baord_undervoltage =
      voltage_vcc < ERRORS_MIN_VCC_VOLTAGE ? true : false;

  // can errors are handled in the handle_can_rx function

  error_data.controler_motor_limit_position =
      movement_controler.get_limit_position_achieved();
}

void config_tasks() {

  can_controler.add_callback(config.can_konarm_clear_errors_frame_id,
                             can_callback_clear_errors);
  can_controler.add_callback(config.can_konarm_set_control_mode_frame_id,
                             can_callback_set_control_mode);
  can_controler.add_callback(config.can_konarm_get_errors_frame_id,
                             can_callback_get_errors);
  can_controler.add_callback(config.can_konarm_status_frame_id,
                             can_callback_status);
  can_controler.add_callback(config.can_konarm_set_pos_frame_id,
                             can_callback_set_pos);
  can_controler.add_callback(config.can_konarm_get_pos_frame_id,
                             can_callback_get_pos);
  can_controler.add_callback(stmepic::CanControl<>::CAN_DEFAULT_FRAME_ID,
                             can_callback_default);

  auto s = stmepic::Timing::Make(
      stmepic::frequency_to_period_us(TIMING_READ_TEMPERATURE_FQ), true,
  ...
  task_blink_error_timer =
      stmepic::Timing::Make(
          stmepic::frequency_to_period_us(TIMING_LED_ERROR_BLINK_FQ), true,
          task_blink_error)
          .valueOrDie();
  task_encoder_timer = stmepic::Timing::Make(stmepic::frequency_to_period_us(
                                                 TIMING_ENCODER_UPDATE_FQ),
                                             true, task_encoders)
                           .valueOrDie();

  task_usb_timer = stmepic::Timing::Make(stmepic::frequency_to_period_us(
                                             TIMING_USB_RECIVED_DATA_FQ),
                                         true, task_usb_handler)
                       .valueOrDie();

  task_data_usb_send_timer =
      stmepic::Timing::Make(
          stmepic::frequency_to_period_us(TIMING_USB_SEND_DATA_FQ), true,
          task_usb_data_loging)
          .valueOrDie();

  task_can_disconnected_timer =
      stmepic::Timing::Make(TIMING_CAN_DISCONNECTED_PERIOD, false,
                            task_can_disconnect)
          .valueOrDie();
  task_read_analog_values_timer =
      stmepic::Timing::Make(
          ...
          .valueOrDie();
  task_nodelay_timer =
      stmepic::Timing::Make(0, true, task_nodelay).valueOrDie();
