...
int encoder_read_raw_angle(encoder_encoder *encoder){
  ...
  HAL_StatusTypeDef status = HAL_I2C_IsDeviceReady(encoder->hi2c, encoder->address, 1, 100);
  if(status != HAL_OK) return -1;
  status = HAL_I2C_Mem_Read(encoder->hi2c, encoder->address, encoder->angle_register, 1, encoder->data, 2, 100);
